...
import { sendReminderEmailAction } from "@/lib/actions";
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const notice = await sendReminderEmailAction(formData);
    ...
    setInvoiceActionNotice(
      redirectUrl,
      buildInvoiceActionErrorNotice(error, "Demo reminder-email action failed.")
    );
  }
