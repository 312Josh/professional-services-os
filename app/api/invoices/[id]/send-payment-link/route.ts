...
import { sendPaymentLinkEmailAction } from "@/lib/actions";
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const notice = await sendPaymentLinkEmailAction(formData);
    ...
    setInvoiceActionNotice(
      redirectUrl,
      buildInvoiceActionErrorNotice(error, "Demo payment-link email action failed.")
    );
  }
